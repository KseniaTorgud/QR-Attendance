...
def test_student_can_register_by_valid_qr(db):
    ...
    assert registration.full_name == "student_qr"
    assert registration.group == ""
    assert response.data["full_name"] == "student_qr"
    assert response.data["group"] == ""


def test_student_can_register_by_qr_with_full_name_and_group(db):
    teacher = User.objects.create_user("teacher_qr_meta", password="securepass123", role=User.UserRole.TEACHER)
    student = User.objects.create_user("student_qr_meta", password="securepass123", role=User.UserRole.STUDENT)
    event = create_open_event(teacher)
    client = auth_client(student)
    ...
    assert response.status_code == status.HTTP_201_CREATED
    registration = Registration.objects.get(student=student, event=event)
    assert registration.full_name == "Ivan Petrov"
    assert registration.group == "CS-101"
    assert response.data["full_name"] == "Ivan Petrov"
    assert response.data["group"] == "CS-101"
